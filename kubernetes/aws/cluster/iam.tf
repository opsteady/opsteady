...
resource "aws_iam_role_policy_attachment" "eks_worker_node" {
  ...
  role       = aws_iam_role.eks_system_node_group.name
}
  ...
  "Statement": [
    {
      "Effect": "Allow",
      "Principal": {
        "Federated": "${aws_iam_openid_connect_provider.platform.arn}"
      },
      "Action": "sts:AssumeRoleWithWebIdentity",
      "Condition": {
        "StringEquals": {
          "${replace(aws_iam_openid_connect_provider.platform.url, "https://", "")}:sub": "system:serviceaccount:kube-system:aws-node"
        }
      }
    }
  ]
