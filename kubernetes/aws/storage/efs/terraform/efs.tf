resource "aws_efs_file_system" "platform" {
  creation_token = "efs-platform-${var.foundation_aws_name}"
...
resource "aws_security_group" "efs" {
  name        = "efs-cluster-${var.foundation_aws_name}"
...
resource "aws_efs_mount_target" "pods_a_subnet" {
  ...
  subnet_id       = var.foundation_aws_pods_a_subnet_id
  security_groups = [aws_security_group.efs.id]
}

resource "aws_efs_mount_target" "pods_b_subnet" {
  file_system_id  = aws_efs_file_system.platform.id
  subnet_id       = var.foundation_aws_pods_b_subnet_id
  security_groups = [aws_security_group.efs.id]
}

resource "aws_efs_mount_target" "pods_c_subnet" {
  file_system_id  = aws_efs_file_system.platform.id
  subnet_id       = var.foundation_aws_pods_c_subnet_id
  security_groups = [aws_security_group.efs.id]
}
