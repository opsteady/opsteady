...
resource "azurerm_role_assignment" "dns_resource_group_reader" {
  ...
  scope                = "/subscriptions/${data.azurerm_client_config.current.subscription_id}/resourcegroups/${var.azure_foundation_resource_group}"
}
...
resource "azurerm_role_assignment" "dns_zone_contributor" {
  ...
  principal_id         = azurerm_user_assigned_identity.dns.principal_id
  scope                = var.azure_foundation_public_zone_id
}
